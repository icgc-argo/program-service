...
class ValidationServiceTest {
  ...
  int init() {
    ...
    when(programService.listCancers()).thenReturn(cancers("Blood cancer", "Brain cancer", "Renal cancer"));
    ...
    this.validationService = new ValidationService(programService, new ValidationProperties().factory());

    return 1;
  }

  private List<CancerEntity> cancers(String... names) {
    return mapToList(names,
      s -> {
  ...
  void createProgramEmptyRequest() {
    ...
    createProgramExpectingErrorMessage(request,
      "No program in the CreateProgram request, A program must have at least one administrator");
  }

  @Test
  void createProgramNoAdmins() {
    val request = CreateProgramRequest.newBuilder().setProgram(goodProgram()).addAllAdmins(admins()).build();
  ...
  void createProgramInvalidEmail() {
    val request = CreateProgramRequest.newBuilder().setProgram(goodProgram()).
      addAllAdmins(admins("invalid")).build();
    createProgramExpectingErrorMessage(request,
      "Invalid email address 'invalid' for admin 'Not Important'");
  ...
  void createProgramEmptyProgram() {
    ...
    val request = CreateProgramRequest.newBuilder().setProgram(program).
      addAllAdmins(admins("valid@test.com")).build();

    createProgramExpectingErrorMessage(request, "commitmentDonors must not be null, "
  ...
  void createProgramBadCancerTypes() {
    ...
    val request = CreateProgramRequest.newBuilder().setProgram(program).
      addAllAdmins(admins("valid@test.com")).build();
    createProgramExpectingErrorMessage(request,
      "Invalid cancerType 'Bad name', Invalid cancerType 'Not cancer'");
  }

  @Test
  void createProgramBadPrimarySites() {
    val program = goodProgram().toBuilder().clearPrimarySites().
      addAllPrimarySites(List.of("Brain", "Blood", "Invalid", "Liver", "Onions")).build();

    val request = CreateProgramRequest.newBuilder().setProgram(program).
      addAllAdmins(admins("valid@test.com")).build();
    createProgramExpectingErrorMessage(request,
      "Invalid primarySite 'Invalid', Invalid primarySite 'Onions'");
  }

  @Test
  void createProgramBadCountries() {
    val program = goodProgram().toBuilder().clearCountries().
      addAllCountries(List.of("Canada", "New Freedonia", "Denmark")).build();

    val request = CreateProgramRequest.newBuilder().setProgram(program).
      addAllAdmins(admins("valid@test.com")).build();
    createProgramExpectingErrorMessage(request,
      "Invalid country 'New Freedonia'");
  }

  @Test
  void createProgramBadRegions() {
    val program = goodProgram().toBuilder().
      addAllRegions(List.of("North America", "Europa")).build();

    val request = CreateProgramRequest.newBuilder().setProgram(program).
      addAllAdmins(admins("valid@test.com")).build();
    createProgramExpectingErrorMessage(request,
      "Invalid region 'Europa'");
  }
}