            ...
            steps {
                ...
                container('curl') {
                    ...
                    withCredentials([string(credentialsId:'JenkisApiToken', variable: 'JenkisApiToken'),
                            ...
                            sh "curl -u 'jenkins-bot:${JenkisApiToken}' -v -X POST 'https://jenkins.qa.cancercollaboratory.org/job/ARGO/job/provision/job/program-service/buildWithParameters?AP_ARGO_ENV=qa&AP_ARGS_LINE=--set%20image.tag%3D${commit}&token=${JenkisApiToken}'"
                    }
                }
            }
        }
